...
<div class="row">
    ...
    <div class="row">
            <div class="col-12 col-md-12">
                <div class="col-md-12">
                    <form action="{{ url('/admin/singlepage/concursoymuestra/'.$concursoymuestra->id) }}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="row form-group">
                            <div class="col-md-12 {{ $errors->has('title') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="title" id="title" class="form-control" value="{{ $concursoymuestra->titulo }}">
                                @if ($errors->has('title'))
                        ...
                        <div class="row form-group">
                            <div class="col-md-9 {{ $errors->has('slug') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="slug" id="slug" class="" value="{{ $concursoymuestra->slug }}" size="50">
                                @if ($errors->has('slug'))
                                    <span class="form-error">
                                    {{ $errors->first('slug') }}
                                    </span>
                        ...
                        <div class="row form-group">
                            <div class="col-md-12 {{ $errors->has('imgportada') ? ' has-error' : '' }}">
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="thumbnail" class="form-control" type="text" name="imgportada" value="{{ $concursoymuestra->image_path }}">
                                </div>
                            </div>
                            <img id="holder" style="margin-top:15px;max-height:100px;" src="{{ asset($concursoymuestra->image_path) }}">
                        </div>
                        ...
                        <div class="row form-group">
                            <div class="col-md-12 {{ $errors->has('lugar') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="lugar" id="lugar" class="form-control" value="{{ $concursoymuestra->lugar }}">
                                @if ($errors->has('lugar'))
                                    <span class="form-error">
                                        {{ $errors->first('lugar') }}
                                    </span>
                        ...
                        <div class="row form-group">
                            <div class="col-md-12 {{ $errors->has('anio') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="anio" id="anio" class="form-control" value="{{ $concursoymuestra->anio }}">
                                @if ($errors->has('anio'))
                                    <span class="form-error">
                                        {{ $errors->first('anio') }}
                                    </span>
                        ...
                        <div class="row form-group-">
                            <div class="col-md-12 {{ $errors->has('content') ? ' has-error' : '' }}">
                                ...
                                <textarea name="content" id="content" cols="30" class="form-control textarea" rows="40">{{ $concursoymuestra->resenia }}</textarea>
                ...
                <div class="col-12 col-md-12">
                    <table id="sort" class="table table-striped grid">
                        ...
                        <tbody>
                        @foreach($concursosymuestrasimagenes as $concursoymuestrasimagen)
                            <tr style="
                                overflow: hidden;
                                text-overflow: ellipsis;">
                                <td>{{$concursoymuestrasimagen->id}}</td>
                                <td>{{$concursoymuestrasimagen->slug}}</td>
                                <td>
                                    <div>
                                        <img src="{{ asset($concursoymuestrasimagen->image_path)}}" class="img-thumbnail" width="20%" alt="{{$concursoymuestrasimagen->nombre}}">
                                    </div>
                                </td>
                                <td class="index">{{$concursoymuestrasimagen->orden}}</td>
                                <td>
                                    <label class="switch">
                                        <input class="estado" type="checkbox" @if($concursoymuestrasimagen->estado) checked @endif>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>
                                    <a href="{{url('/admin/singlepage/concursoymuestra')}}/{{$concursoymuestrasimagen->id}}/deleteimagen" class="btn btn-xs btn-primary" ><i class=""></i> Borrar</a>
                                    {{--</div>--}}
                                </td>
                            </tr>
                        @endforeach
...
<script>
        ...
        updateIndex = function(e, ui) {
            $('td.index', ui.item.parent()).each(function (i) {
                ...
                $.ajax({
                    ...
                    url: '{{ route('ordenarConcursosYMuestrasInterior') }}', // This is the url we gave in the route
    ...
    $(".estado").change(function(){
        if($(this).prop("checked") == true){
            ...
            $.ajax({
                type: 'get', // Type of response and matches what we said in the route
                url: '{{ route('cambiarEstadoConcursosYMuestrasInterior') }}', // This is the url we gave in the route
